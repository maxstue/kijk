...
12. **Do not block commits solely because Fallow fails to execute**. If Fallow cannot install, crashes, cannot create a temporary worktree, or reports an external environment issue, committing is still allowed. Include a clear warning in the final response with the failed command and reason.
13. **Do not block unrelated commits solely because Fallow reports inherited findings**. Review findings related to the current change, but report inherited findings as a warning and continue with the remaining verification.
