...
const itemVariants = tv({
  base: 'rounded-md px-2 py-1 transition-colors hover:text-accent-foreground',
  variants: {
    active: {
...
export function FumadocsIcon(props: React.SVGProps<SVGSVGElement>): React.ReactElement {
  const id = useId();
  return (
    <svg width='80' height='80' viewBox='0 0 180 180' filter={`url(#${id}-shadow)`} {...props}>
      <circle cx='90' cy='90' r='90' fill={`url(#${id}-iconGradient)`} />
      <defs>
        <filter id={`${id}-shadow`} colorInterpolationFilters='sRGB'>
          <feDropShadow dx='0' dy='0' stdDeviation='1' floodColor='hsl(var(--primary))' floodOpacity='1' />
        </filter>
